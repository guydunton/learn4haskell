...
-}
subList :: Int -> Int -> [a] -> [a]
subList from to values
  | from < 0 || to < 0 = []
  | otherwise = drop from $ take (to + 1) values
...
duplicate vals =
  ...
  where
    go :: [a] -> [a]
    go (front : back) = front : front : (go back)
    go _              = []
...
-}
contains :: Eq a => a -> [[a]] -> [[a]]
contains element vals =
  filter (\v -> elem element v) vals
...
rotate index vals
  | index < 0 = []
  | otherwise = take (length vals) $ drop index $ cycle vals
